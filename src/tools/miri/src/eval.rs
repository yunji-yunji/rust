...
// use crate::fuzz::_generate_path4;
...
fn mir_to_petgraph<'tcx>(_tcx: TyCtxt<'tcx>, body: &Body<'_>, arr: &mut Vec<NodeIndex>, scc_info_stk: &mut FxHashMap<NodeIndex, Vec<SccInfo>>)
                         -> Graph::<usize, String>{
...
pub fn transform_to_single_header<'tcx>(scc: &mut Vec<NodeIndex>,
    ...
    // let bbs = body.as_ref().map(|a|{a.})
    let bbs = body.clone().basic_blocks;

    for header in headers {

        // let h = bbs.get_mut(BasicBlock::from_usize(header.index())).expect("no bb in mir");
        let h = bbs.get(BasicBlock::from_usize(header.index())).expect("no bb in mir");
        println!("header {:?}", h);
        ...
        // print_bbs_mut(bbs, "Get single hedaer2");
    }
...
pub fn transform_to_single_latch(scc: &mut Vec<NodeIndex>,
    ...
    let new_node;
    if back_edges.len() > 1 {
        println!("####### back edges2 {:?}", back_edges);
        ...
        // print_bbs(body.clone().basic_blocks, "In get single latch2");
...
pub fn mark<'tcx>(
    ...
    println!("====================== Transform & Mark2 ======================");
    // 1. mark header
    let headers = find_all_headers(scc.clone(), g);
    if headers.len() ==1 {
        println!("[1] if there is a single header2 {:?}", headers);
        loop_header = headers[0];
    } else {
        println!("[2] if there are multiple headers2 {:?}", headers);
    ...
    println!("remove single latch 2 = {:?} -> header = {:?}", single_latch, loop_header);
    g.remove_edge(edge_idx);

    *scc_id += 1;
}

fn mark_scc_info<'tcx>(tcx: TyCtxt<'tcx>, index_map: &mut Vec<NodeIndex>, scc_info_stk: &mut FxHashMap<NodeIndex, Vec<SccInfo>>,
                       body: &Body<'_>) {
    let g = mir_to_petgraph(tcx, body, index_map, scc_info_stk);
    print_bbs(body.clone().basic_blocks, "Initial MIR 2");
...
pub fn eval_entry<'tcx>(
    ...
    for (n_idx, &ref stack) in scc_info_stk.iter() {
        println!("node check 3: {:?} == {:?}", n_idx, stack);
    }
