...
//
/*
pub fn _generate_path4(_g: Graph::<usize, String>,
                      scc_info_stk: &mut HashMap<NodeIndex, Vec<SccInfo>>,
                      arr: Vec<NodeIndex>,
path : Vec<usize>) -> Vec<i32> {
    #[derive(Debug)]
    struct Ele {
        counts: HashMap<Vec<i32>, usize>,
        temp_path: Vec<Vec<i32>>,
    ...
    println!("============= In Generate Path ================");
    println!("path INFO: {:?} {:?} ", path.len(), path.clone());
    fin.push(path[0] as i32);
    ...
    for idx in 0..path.len() - 1 {
        ...
        while t_idx < scc_info_stk[&arr[t]].len() {
            ...
            // println!("^^ {:?} {:?} {:?}", el.counts, el.prefix, el.temp_path);
...
// }
//
// pub fn _generate_path(g: &mut Graph::<usize, String>, _new_g: &mut Graph::<usize, String>,
