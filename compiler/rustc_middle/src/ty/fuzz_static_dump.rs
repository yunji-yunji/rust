...
/*
        match self.krate.as_deref() {
            ...
            Some(k_name) => match k_name {
                ...
                // "panic_unwind" | "getopts" | "rand" | 
                "core" | "std" | "alloc" | "backtrace" | "hashbrown" | "petgraph" | "bcs" | "test" => {
...
impl<'sum, 'tcx> PaflDump<'sum, 'tcx> {
            ...
            TerminatorKind::Call {
                ...
                match func.constant() {
                    None => {
                        ...
                        // TODO: should panic?
                        println!("[RUSTC] callee is not a constant: type kind [{:?}]", cty.kind());
            ...
            TerminatorKind::TailCall { 
                ...
                match func.constant() {
                    None => {
                        ...
                        // TODO: should panic?
                        println!("[RUSTC] callee is not a constant (tailcall): type kind [{:?}]", cty.kind());
                        TermKind::Unreachable
                    },
                    Some(_) => {
                        TermKind::TailCall {
    ...
    pub fn process_cfg(&mut self, _id: DefId, body: &Body<'tcx>) -> PaflCFG {
        // Disable cfg dump to data directory for now
        /*
        ...
        */
        ...
        // local_decls is needed for process_block
    ...
    pub fn summarize_instance(
        ...
        // TODO: do we need this? (disable for now, change to conditional compile)
        // TODO: disable cfg dump as well for now (use for path_prefix -> use for dump CFG plots)
...
impl<'tcx> TyCtxt<'tcx> {
    pub fn static_dump(self, outdir: &Path) {
...
*/
        ...
        for unit in units {
            ...
            for item in unit.items().keys() {
                ...
                // This is removed because we run static_dump in MIRI
